...
def main() -> int:
    ...
    rows = (db._client.table("pipeline_health")
            .select("*")
            .eq("workflow", "eod")
            .eq("status", "SUCCESS")
    ...
    if not rows:
        ...
        db.upsert("pipeline_health", [{
            "run_id": str(uuid.uuid4()), "workflow": "eod",
            "status": "DELAYED", "started_at": now.isoformat(),
            "finished_at": now.isoformat(), "duration_seconds": 0,
            "rows_processed": 0, "error_message": "no EOD run ever recorded",
        }], on_conflict="run_id")
        return 1
